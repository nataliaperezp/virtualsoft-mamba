"""
launch_sessions.py — Lanzador de experimentos Mamba-Sessions en Vertex AI
==========================================================================
Uso:
  python3 launch_sessions.py                  # modo interactivo
  python3 launch_sessions.py --list           # lista los experimentos
  python3 launch_sessions.py --run 1,3,5      # lanza los experimentos 1, 3 y 5
  python3 launch_sessions.py --run all        # lanza todos
  python3 launch_sessions.py --dry-run --run 2  # imprime el YAML sin ejecutar
  python3 launch_sessions.py --test --run 1   # prueba rápida (~1000 usuarios, 2 trials)

  # Modo estático (sin Optuna, hiperparámetros fijos):
  python3 launch_sessions.py --static --run 5 \\
...
# CONFIGURACIÓN GCP
# ---------------------------------------------------------------------------
ESTADO         = "MambaModel_Sessions_v1"
PROJECT_ID     = "composed-arch-276322"
DEFAULT_REGION = "us-east1"
IMAGE          = f"gcr.io/{PROJECT_ID}/mamba-sessions:v1"

# OUT_DIR apunta al directorio de sessions — el script usa glob batch_*.parquet
OUT_DIR        = "gs://ml-bucketvs/mamba-exp/data/sequences_sessions_v2/sequences_output_sessions_v5"

# Perfiles de máquinas preconfigurados
MACHINE_PROFILES = {
    "g16":  {"machine_type": "g2-standard-16",      "accelerator": "nvidia-l4",         "num_gpus": 1, "default_workers": 6,  "desc": "1x L4 (24GB VRAM, 64GB RAM)"},
    ...
    "t4":   {"machine_type": "n1-highmem-8",        "accelerator": "nvidia-tesla-t4",   "num_gpus": 1, "default_workers": 2,  "desc": "1x T4 (16GB VRAM, 52GB RAM)"},
}
DEFAULT_MACHINE = "g16"
...
def get_ckpt_base(accelerator: str, machine_type: str) -> str:
    return f"gs://ml-bucketvs/mamba-exp/checkpoints/{ESTADO}/{accelerator}-{machine_type}"


def print_grid():
    print("\n┌─────────────────────────────────────────────────────────────┐")
    print("│          GRID DE EXPERIMENTOS MAMBA-SESSIONS                │")
    print("├────┬─────────────────┬───────────────┬─────────────────────┤")
...
def build_job_yaml(exp: dict, timestamp: str = "", test_mode: bool = False, test_files: int = 10,
                   ...
                   grad_accum_steps: int = 1, sample_fraction: float = 1.0,
    ...
    wandb_project = f"{ESTADO}-{exp_slug}"
    ...
    env_vars = [
        ...
        {"name": "GRAD_ACCUM_STEPS",        "value": str(grad_accum_steps)},
        {"name": "SAMPLE_FRACTION",         "value": str(sample_fraction)},
...
def launch_experiment(exp: dict, dry_run: bool = False, test_mode: bool = False,
    ...
    exp_slug      = f"e{exp['id']}-{static_suffix}{test_suffix}"
    job_name      = f"mamba-sessions-{exp_slug}-{timestamp}"
    ...
    yaml_content = build_job_yaml(exp, timestamp=timestamp, test_mode=test_mode,
                                  test_files=test_files, group_size=group_size,
                                  n_epochs=n_epochs, batch_size=batch_size,
                                  num_workers=num_workers, n_trials=n_trials,
                                  static_mode=static_mode, d_model=d_model,
                                  d_state=d_state, lr=lr, lambda_cl=lambda_cl,
                                  temp=temp,
    ...
    try:
        run_label  = f"ms{exp['id']}-{ESTADO.lower().replace('_', '-')}-{timestamp.replace('-', '')}"
        labels_str = f"model=mamba-sessions,run_label={run_label},proyecto=sistema-de-recomendacion-dev"
...
def main():
    parser = argparse.ArgumentParser(
        description="Lanzador de experimentos Mamba-Sessions en Vertex AI",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=textwrap.dedent("""\
            Ejemplos:
              python3 launch_sessions.py                         # modo interactivo
              python3 launch_sessions.py --list                  # lista experimentos
              python3 launch_sessions.py --run 1,3               # lanza exp 1 y 3
              python3 launch_sessions.py --run all               # lanza todos
              python3 launch_sessions.py --dry-run --run 2       # sin ejecutar (muestra el YAML)
              python3 launch_sessions.py --test --run 1          # prueba rápida
        """)
    )
    parser.add_argument("--list",       action="store_true", help="Lista el grid y sale")
    parser.add_argument("--run",        type=str, default=None, help="IDs a lanzar (ej: 1,3,5 o 'all')")
    parser.add_argument("--run-group",  type=str, default=None, help="Prefijo para agrupar este lanzamiento")
    ...
    parser.add_argument("--machine",    type=str, default=DEFAULT_MACHINE, choices=MACHINE_PROFILES.keys(),
                        help=f"Perfil de máquina (default: {DEFAULT_MACHINE}). Opciones: " +
                             ", ".join(f"{k}={v['desc']}" for k, v in MACHINE_PROFILES.items()))
    ...
    parser.add_argument("--test-files",  type=int, default=10,
                        help="Nro. archivos parquet en modo prueba (default: 10)")
    ...
    parser.add_argument("--batch-size",  type=int, default=0,
                        help="Batch size (default: 16 prueba / 64 prod)")
    parser.add_argument("--num-workers", type=int, default=0,
                        help="Workers del DataLoader (default: según perfil de máquina)")
    parser.add_argument("--trials",       type=int, default=0,
                        help="Trials de Optuna (default: 2 prueba / 15 prod)")
    parser.add_argument("--grad-accum",     type=int,   default=1,
                        help="Gradient accumulation steps (batch efectivo = batch x grad_accum, default: 1)")
    parser.add_argument("--sample-fraction",type=float, default=1.0,
                        help="Fraccion de UIDs a usar 0.0-1.0 (default: 1.0; usar 0.25 para HPO rapido)")
    # Modo estático
    parser.add_argument("--static",      action="store_true",
                        help="Modo estático: entrena con hiperparámetros fijos, sin Optuna")
    parser.add_argument("--d-model",     type=int,   default=256,  help="d_model (modo estático)")
    parser.add_argument("--d-state",     type=int,   default=64,   help="d_state (modo estático)")
    parser.add_argument("--lr",          type=float, default=5e-4, help="learning rate (modo estático)")
    parser.add_argument("--lambda-cl",   type=float, default=0.5,  help="lambda contrastive (modo estático)")
    parser.add_argument("--temp",        type=float, default=0.1,  help="temperatura contrastive (modo estático)")
    ...
    print(f"   Config efectiva: epochs={eff_epochs} | batch={eff_batch_size} | group_size={eff_group_size} | trials={eff_trials} | workers={eff_workers}", end="")
    if test_mode:
    ...
    for exp in selected_exps:
        launch_experiment(exp, dry_run=args.dry_run, test_mode=test_mode,
